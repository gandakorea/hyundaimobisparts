...
해당 Vercel 프로젝트에 `MOBIS_PUBLIC_SYNC=true`를 설정하고 재배포하면
접속 즉시 기존 주문 데이터를 불러오며 기기 연결 코드 입력란이 나타나지 않습니다.
이 설정은 주소를 아는 누구나 주문을 조회·수정할 수 있게 합니다.
현대 주문장에만 적용하려면 현대 프로젝트에만 설정합니다.
대성 프로젝트는 이 값을 설정하지 않으면 기존 연결 코드 인증을 유지합니다.

코드 인증을 다시 사용하려면 이 변수를 삭제하거나 `false`로 바꾼 후 재배포합니다.
`MOBIS_SYNC_CODE`와 세션 설정은 그대로 보관하며, 자동 연결 모드에서는 사용자가
코드를 입력할 필요가 없습니다. 기존 주문 데이터와 데이터베이스 구조는 변경하지 않습니다.
...
2. 공유 버튼을 누릅니다.
3. `홈 화면에 추가`를 선택합니다.
4. 코드 인증 모드라면 처음 실행한 뒤 기기 연결 코드를 입력합니다. 자동 연결 모드에서는 생략합니다.
